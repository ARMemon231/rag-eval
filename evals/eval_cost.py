...
REPEATS = 3       # cost is near-deterministic with temp=0, 1 repeat avoids Mistral rate limits

# --- Pricing: gemini-3.5-flash-lite, USD per 1M tokens (verified Aug 2026). ---
...
def measure_tokens(pipeline, question):
    ...
    time.sleep(2.0)  # Pause between reranker and generator
    ...
    for attempt in range(5):
        try:
            ...
            if "429" in str(e) and attempt < 4:
                wait = 3 * (2 ** attempt)
                print(f"  ⏳ Rate limited on generation, retrying in {wait}s...")
                time.sleep(wait)
            else:
                raise
...
def benchmark(pipeline):
    ...
    for question in QUESTIONS:
        for _ in range(REPEATS):
            ...
            time.sleep(2.0)  # Cooldown between queries
    return rows
...
def report(rows):
    ...
    print("\n" + "=" * 70)
    print(f"COST  (gemini-3.5-flash-lite @ ${PRICE_INPUT_PER_1M}/${PRICE_OUTPUT_PER_1M} per 1M in/out)")
    print("=" * 70)
    print(f"samples                : {n}")
    print(f"avg input tokens       : {avg_in:8.0f}   ({avg_cached:.0f} cached)")
    print(f"avg output tokens      : {avg_out:8.0f}")
    print("-" * 70)
    print(f"avg cost / query       : ${avg_cost:.6f}   (Rs {avg_cost * USD_TO_PKR:.4f})")
    ...
    print(f"   per day             : ${daily:8.2f}   (Rs {daily * USD_TO_PKR:8.2f})")
    print(f"   per month           : ${monthly:8.2f}   (Rs {monthly * USD_TO_PKR:8.2f})")
    print("=" * 70)
