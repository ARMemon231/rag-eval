...
# --- generate ---
chunks = load_chunks()
sample = random.sample(chunks, min(10, len(chunks)))  # keep small to respect rate limits
contexts = [[c] for c in sample]                       # each context = one chunk

mistral_model = MistralDeepEvalModel(model_name="mistral-medium-latest")

synthesizer = Synthesizer(model=mistral_model)          # pass the wrapper, not a string
...
print(f"Wrote {len(rows)} DRAFT goldens -> goldens/retriever_deepeval_goldens.json")
